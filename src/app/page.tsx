...
const GlobalStyles = () => (
  <style>{`
    ...
    /* Project Card Custom Shadows */
    .shadow-emerald-glow { box-shadow: 0 10px 40px -10px rgba(16,185,129,0.35); }
    .shadow-blue-glow { box-shadow: 0 10px 40px -10px rgba(59,130,246,0.35); }
    .shadow-amber-glow { box-shadow: 0 10px 40px -10px rgba(245,158,11,0.35); }
    .shadow-violet-glow { box-shadow: 0 10px 40px -10px rgba(139,92,246,0.35); }
  ...
  return (
    <div className="min-h-screen relative selection:bg-emerald-500 selection:text-white">
      ...
      <AnimatePresence>
        {selectedCert && (
          <motion.div 
            ...
            className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-10 bg-neutral-900/80 backdrop-blur-sm cursor-zoom-out"
          >
            <motion.div 
              initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }}
              transition={{ type: "spring", damping: 25, stiffness: 300 }}
              onClick={(e) => e.stopPropagation()} 
              className="relative max-w-5xl w-full bg-white/95 backdrop-blur-2xl rounded-3xl overflow-hidden shadow-2xl cursor-default border border-white/50"
            >
              ...
              <iframe src={`${selectedCert.img}#view=FitH`} title={selectedCert.title} className="w-full h-[75vh] border-none bg-neutral-100/50" />
              <div className="p-5 bg-white/80 backdrop-blur-md text-center border-t border-neutral-100 relative z-10">
                <h3 className="font-bold text-lg text-neutral-900">{selectedCert.title}</h3>
                <p className="text-sm text-neutral-500">{selectedCert.issuer}</p>
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      <section id="about" className="py-24 md:py-32 relative overflow-hidden bg-white border-t border-neutral-100 min-h-screen flex items-center">
        ...
        <motion.div animate={{ x: [0, -60, 60, 0], y: [0, 60, -60, 0], scale: [1, 0.8, 1.2, 1] }} transition={{ duration: 18, repeat: Infinity, ease: "easeInOut", delay: 2 }} className="absolute bottom-1/4 right-0 w-[600px] h-[600px] bg-teal-200/20 rounded-full blur-[150px] -z-10 pointer-events-none" />
              ...
              <BlurFade delay={0.2} className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="relative group rounded-[2rem] p-[2px] overflow-hidden cursor-pointer shadow-sm hover:shadow-[0_0_40px_-10px_rgba(14,165,233,0.3)] transition-shadow duration-500">
                    ...
                    <div className="relative h-full w-full bg-white/90 backdrop-blur-xl rounded-[2rem] p-6 flex flex-col justify-center gap-4 z-10 border border-white/50 group-hover:bg-white/95 transition-colors">
                      ...
                      <div><p className="text-xs text-neutral-400 font-bold uppercase tracking-widest mb-1">GPA</p><p className="text-xl font-bold text-neutral-900 leading-tight flex items-baseline gap-1.5">3.65 <span className="text-sm font-medium text-neutral-400 normal-case tracking-normal"></span></p></div>
                    </div>
                </div>
              </BlurFade>
            </div>
            <BlurFade delay={0.3} className="relative w-full max-w-lg mx-auto lg:mx-0 perspective-1000">
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-[radial-gradient(circle,rgba(16,185,129,0.15)_0%,transparent_60%)] -z-10 animate-pulse"></div>
      ...
      <section id="contact" className="py-32 relative overflow-hidden bg-[#FAFAFA] border-t border-neutral-100 min-h-screen flex items-center">
        ...
        <motion.div animate={{ x: [0, 40, -40, 0], y: [0, -40, 40, 0], scale: [1, 1.1, 0.9, 1] }} transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }} className="absolute top-0 right-0 w-[400px] h-[400px] bg-emerald-300/20 rounded-full blur-[100px] -z-10 pointer-events-none" />
        <motion.div animate={{ x: [0, -50, 50, 0], y: [0, 50, -50, 0], scale: [1, 0.9, 1.1, 1] }} transition={{ duration: 15, repeat: Infinity, ease: "easeInOut", delay: 2 }} className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-teal-300/20 rounded-full blur-[120px] -z-10 pointer-events-none" />
